...
RUN apt-get -q update \
 ...
 && echo 'execute(){ local -; executable="$(find /fme-shared/AutoExecute -maxdepth 1 -type f -print -quit)"; if [ -n "$executable" ]; then echo "Executing $executable"; set -x; "$executable"; mv -f "$executable" /fme-shared/AutoExecute/Finished/; fi; sleep 1; }; while true; do execute; done' >> /usr/local/bin/autoexecute \
