...
RUN apt-get -q update \
 && apt-get -qy --no-install-recommends install zip libtcmalloc-minimal4 libqt5core5a libboost-system1.65.1 libboost-thread1.65.1 libtcl8.5 libjpeg-turbo8 libfreetype6 libssl1.1 libssh2-1 libboost-filesystem1.65.1 libboost-regex1.65.1 libboost-iostreams1.65.1 libcgroup1 libqt5network5 libboost-locale1.65.1 libsqlite3-0 libmpfr6 libexpat1 libxml2 libqt5sql5 openjdk-8-jre-headless odbc-postgresql libpython3.6 \
 ...
 && echo 'while true; do executable="$(find /fme-shared/AutoExecute -maxdepth 1 -type f -print -quit)"; if [ -n "$executable" ]; then date; bash -v "$executable"; mv -f "$executable" /fme-shared/AutoExecute/Finished/; touch "/fme-shared/AutoExecute/Finished/$(basename "$executable")"; date; fi; sleep 1; done' >> /usr/local/bin/autoexecute \
 ...
 && chmod ug=rx,o= /usr/local/bin/autoexecute \
